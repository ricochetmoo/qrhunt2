...
export function ProgressBar({
  ...
  return (
    <div className={cn("w-full", className)}>
      {labelPosition === "above" ? <p className="mb-1 text-sm font-bold text-black">{visibleLabel}</p> : null}
      <div
        role="progressbar"
        aria-valuemin={0}
      ...
      >
        ...
        {labelPosition === "inside" ? (
          <span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-black mix-blend-multiply">
            {visibleLabel}
          </span>
        ) : null}
      </div>
      {labelPosition === "below" ? <p className="mt-1 text-sm text-scouts-grey-dark">{visibleLabel}</p> : null}
    </div>
  );
}
