  ...
  return (
    <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {progress.map(({ team, scans }) => {
        ...
        return (
          <li key={team.id} className="rounded-lg border border-slate-200 bg-white p-3">
            <p className="text-sm font-medium text-slate-900">{team.name}</p>
            {last ? (
              <div className="mt-1 flex items-center gap-2 text-xs text-slate-500">
                <span>{last.code.name}</span>
                <span title={new Date(last.scan.createdAt).toLocaleString()}>
              ...
              <p className="mt-1 text-xs text-slate-400">No scans yet</p>
            )}
          </li>
        );
      })}
